...
def _workspace_caller() -> dict[str, Any]:
    for fr in inspect.stack(context=0)[2:]:
        ...
        if p.startswith(ROOT + "/"):
            return {"file": fr.filename, "line": fr.lineno, "caller": fr.function}
...
def _ret(v: Any) -> Any:
    if isinstance(v, BrokerSession):
        return {"type": "BrokerSession", "id": id(v), "access_token_length": len(v.access_token or "")}
...
def patched_init(self, *args, **kwargs):
    c = _workspace_caller()
    first_init = not bool(getattr(self, "_initialized", False))
    trace["init_calls"].append(
        {
            "id(self)": id(self),
            "file": c["file"],
            "line": c["line"],
            "caller": c["caller"],
            "first_time_init": first_init,
        }
    )
...
def main() -> None:
    ...
    if hasattr(FivePaisaSessionManager, "save_secure"):
        trace["notes"].append("save_secure exists")
    else:
        trace["notes"].append("save_secure not present; traced _store_secure")
    ...
    try:
        page = PortfolioSummaryPage(None)
        page.refresh_data()
    except Exception:
        pass
