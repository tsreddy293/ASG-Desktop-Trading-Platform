...
REQUEST_TOKEN = "TRACE_ONLY_DUMMY_REQUEST_TOKEN"
    ...
    def wrapped(self, *args, **kwargs):
        try:
            if method_name == "exchange_request_token":
                ...
                trace_report["started_post_token"] = True
...
def _install_oauth_bypass() -> None:
    original = FivePaisaBrokerClient._authenticate_via_oauth_flow
    ORIGINALS["FivePaisaBrokerClient._authenticate_via_oauth_flow"] = original

    def bypass(self):
        self.reauthenticate_from_request_token(REQUEST_TOKEN)

    FivePaisaBrokerClient._authenticate_via_oauth_flow = bypass


def _install_exchange_success_stub() -> None:
    original = FivePaisaAuthService._post_json
    ORIGINALS["FivePaisaAuthService._post_json"] = original

    def stub_post_json(url: str, payload: dict[str, Any]) -> dict[str, Any]:
        return {
            "body": {
                "AccessToken": "TRACE_ACCESS_TOKEN_" + ("X" * 180),
                "RefreshToken": "TRACE_REFRESH_TOKEN_" + ("Y" * 180),
                "ClientCode": "52828064",
                "Status": 0,
                "Message": "Success",
            },
    ...
    FivePaisaAuthService._post_json = staticmethod(stub_post_json)
...
def install_tracing() -> None:
    ...
    _install_oauth_bypass()
    _install_exchange_success_stub()
